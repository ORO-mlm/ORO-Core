...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/OROParams"
    else
        PARAMS_DIR="$HOME/.oro-params"
    fi
fi
...
function main() {
    ...
    cat <<EOF
ORO - install-params.sh

This script will install the ORO zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common ORO zkSNARK parameters. Note that it is
