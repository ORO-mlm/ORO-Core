...
QString formatBalanceWithoutHtml(CAmount amount, int nDisplayUnit, bool isZoro)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZoro)) : BitcoinUnits::floorWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZoro);
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter ORO address (e.g. %1)").arg("R7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
fs::path static StartupShortcutPath()
{
    if (gArgs.GetBoolArg("-testnet", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ORO (testnet).lnk";
    else if (gArgs.GetBoolArg("-regtest", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ORO (regtest).lnk";
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=ORO (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=ORO (regtest)\n";
        else
            optionFile << "Name=ORO\n";
