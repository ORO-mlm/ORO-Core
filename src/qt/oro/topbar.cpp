...
#include "qt/oro/balancebubble.h"
...
TopBar::TopBar(OROGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopOro}, "amount-small-topbar");
    setCssProperty({ui->labelAmountOro}, "amount-topbar");
    setCssProperty({ui->labelPendingOro, ui->labelImmatureOro}, "amount-small-topbar");
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // ORO Total
    QString totalOro = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);
    ...
    // ORO
    // Top
    ui->labelAmountTopOro->setText(totalTransparent);
    // ui->labelAmountTopShieldedOro->setText(totalShielded);
    // Expanded
    ui->labelAmountOro->setText(totalOro);
    ui->labelPendingOro->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance + newBalance.unconfirmed_shielded_balance, nDisplayUnit));
    ui->labelImmatureOro->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
