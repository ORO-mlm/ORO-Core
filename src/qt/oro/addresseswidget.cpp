...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto oroAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(oroAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(oroAdd);
        if (walletModel->updateAddressBookLabels(oroAdd, label.toUtf8().constData(),
