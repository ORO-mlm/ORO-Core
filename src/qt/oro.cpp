...
void BitcoinApplication::createWindow(const NetworkStyle* networkStyle)
{
    ...
    connect(pollShutdownTimer, &QTimer::timeout, window, &OROGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &OROGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &OROGUI::handlePaymentRequest);
        connect(window, &OROGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. ORO can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("ORO Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
