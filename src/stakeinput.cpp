...
COroStake* COroStake::NewOroStake(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize COroStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    return new COroStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool COroStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}
...
CAmount COroStake::GetValue() const
{
    return outputFrom.nValue;
}

bool COroStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream COroStake::GetUniqueness() const
{
...
const CBlockIndex* COroStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("COroStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool COroStake::ContextCheck(int nHeight, uint32_t nTime)
{
