...
bool CLegacyZOroStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZOroStake from non zc-spend", __func__);
    ...
    *this = CLegacyZOroStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zoro stake origin", __func__);

    // All good
    return true;
}

CLegacyZOroStake::CLegacyZOroStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZOroStake::GetIndexFrom() const
{
...
CAmount CLegacyZOroStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZOroStake::GetUniqueness() const
{
...
bool CLegacyZOroStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zORO stake block: height %d outside range", __func__, nHeight);
