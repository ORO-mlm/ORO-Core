...
/** Fees smaller than this (in uoro) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k ORO
...
{
    util::ThreadRename("oro-scriptch");
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        ...
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t)consensus.nOroBadBlockTime) &&
            (block.nBits == (uint32_t)consensus.nOroBadBlockBits)) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const auto& txIn : bl.vtx) {
                    ...
                    for (const CTxIn& in : t.vin) {
                        ...
                        if (hasZOROInputs && in.IsZerocoinSpend())
