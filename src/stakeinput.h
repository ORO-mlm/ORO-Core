...
public:
    COroStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static COroStake* NewOroStake(const CTxIn& txin);
    ...
    bool IsZORO() const override { return false; }
