...
UniValue importprivkey(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            "importprivkey \"oroprivkey\" ( \"label\" rescan fStakingAddress )\n"
    ...
    CKeyID vchAddress = pubkey.GetID();
    {
        ...
        if (fRescan) {
            ...
            if (fStakingAddress && !Params().IsRegTestNet()) {
                // cold staking was activated after nBlockTimeProtocolV2 (ORO v4.0). No need to scan the whole chain
...
UniValue importwallet(const JSONRPCRequest& request)
{
    ...
    while (file.good()) {
        ...
        // Sapling keys
        // Let's see if the address is a valid ORO spending key
