ORO Core version v5.1.0 is now available from:  <https://github.com/oro-cm/ORO/releases>
...
In order to run, ORO Core now requires two files, `sapling-output.params` and `sapling-spend.params` (with total size ~50 MB), to be saved in a specific location.
...
For the other packages, the user must save the param files in the proper location, before being able to run ORO v5.0.0:
- macOS/Linux `tar.gz` tarballs include a bash script (`install-params.sh`) to copy the parameters in the appropriate location.
- Windows `.zip` users need to manually copy the files from the `share/oro` folder to the `%APPDATA%\OROParams` directory.
...
- #2249 Make box of ORO Send address return to purple when it's empty (PicklesRcool)
...
- #2170 Better oro.conf Masternode data error notification (furszy)
