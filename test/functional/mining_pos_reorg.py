...
class ReorgStakeTest(OroTestFramework):
    ...
    def check_money_supply(self, expected_oro):
        # verify that nodes have the expected ORO supply
        oro_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(oro_supply, [DecimalAmt(expected_oro)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that ORO supply was properly updated after the reorgs
        self.log.info("Check ORO supply...")
